...
// app/examination/[exam_id]/review/ReviewClient.tsx
...
function Shell({ exam, children }: { exam: Exam; children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-white font-sans text-[#333]">
      ...
      <div className="px-4 py-4 border-b border-[#ddd]">
        ...
        <div className="text-[13px] text-[#777] mt-0.5">
          {exam.questions.length} questions · Review
        </div>
      </div>
      {children}
    </div>
...
export default function ReviewClient({
  ...
  if (!exam.questions || exam.questions.length === 0) notFound();
      ...
      <div className="block md:hidden px-3 py-3">
        <div className="border border-[#ddd]">
          <div className="bg-[#f5f5f5] border-b border-[#ddd] px-3 py-2 flex items-center justify-between">
            ...
            {(isCorrect === null || isCorrect === undefined) && (
              <span className="flex items-center gap-1 text-[12px] text-[#8a6d3b] bg-[#fcf8e3] border border-[#faebcc] px-2 py-0.5">
        ...
        <button
          onClick={() => setPhase("result")}
          className="w-full py-2 text-[14px] text-[#333] bg-[#e8e8e8] border border-[#ccc] rounded mb-4"
        >
          ← Return to results
        </button>
        <Link
          href="/examination"
          className="block w-full py-2 text-center text-[14px] text-[#337ab7] border border-[#337ab7] rounded"
        >
          ← All exams
        </Link>
      </div>
